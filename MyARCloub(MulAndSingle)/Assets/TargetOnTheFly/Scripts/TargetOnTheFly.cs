...
public class TargetOnTheFly : MonoBehaviour
{
    ...
    void OnGUI()
    {
		...
		if (GUI.Button (new Rect (Screen.width  - 300, 200, 300, 100), "Proccess", skin.GetStyle ("Button"))) {
			imageCreater.StartTakePhoto();

		}
            

        if (GUI.Button(new Rect(Screen.width - 300, Screen.height - 200, 300, 100), "Clear", skin.GetStyle("Button")))
        {
            imageCreater.ClearTexture();
			imageCreater.ClearMarkNameList ();

			imageManager.ClearAllTrackerAndTarget();
        }
    }
